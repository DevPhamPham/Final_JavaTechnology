...
import javatechnology.project.configuration.ResourceNotFoundException;
import javatechnology.project.dto.CartDTO;
import javatechnology.project.dto.UserDTO;
import javatechnology.project.model.*;
import javatechnology.project.repository.OrderRepository;
import javatechnology.project.service.*;
...
//        System.out.println("-----debug-----------");
    ...
    public ResponseEntity<List<Product>> searchAndFilter(HttpServletRequest request){
        // Lấy các tham số từ URL
        String category = request.getParameter("category");
        if (category=="") category =null;
        String name = request.getParameter("name");
    ...
    public String account(Model model,HttpSession session) throws ResourceNotFoundException{
        try {
            ...
            if (principal instanceof DefaultOidcUser || principal instanceof CustomUserDetail && principal != null) {
                String currentUserEmail = "";
                try {
                    currentUserEmail = ((CustomUserDetail)principal).getEmail();
                }catch (Exception e){
                    currentUserEmail = ((DefaultOidcUser)principal).getEmail();
                }
